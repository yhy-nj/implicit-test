...
class ExplicitConstraintPreprocessor(BaseDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        # print(f"[DEBUG preprocessor] inputs keys: {list(inputs.keys())}")
        # if data_samples is not None and len(data_samples) > 0:
        #     meta = data_samples[0].metainfo
        #     print(f"[DEBUG preprocessor] metainfo keys: {list(meta.keys())}")
        #     print(f"[DEBUG preprocessor] has img: {'img' in meta}, has lidar2img: {'lidar2img' in meta}")

        # print(f"[DEBUG preprocessor] inputs keys: {list(inputs.keys())}") #4444444444444444444444
        ...
        # Step 2: Process images and compute projections
        # 图像通过 Pack3DDetInputs 的 meta_keys 传入 data_samples.metainfo
        has_images = (data_samples is not None
                      and len(data_samples) > 0
                      and 'img' in data_samples[0].metainfo
                      and 'lidar2img' in data_samples[0].metainfo)
        ...
        if has_images:
            ...
            images, proj_coords = self.process_images_and_project(
                inputs['points'], imgs, data_samples)
            voxel_dict['images'] = images
            voxel_dict['proj_coords'] = proj_coords
            voxel_dict['has_images'] = True
        else:
            voxel_dict['has_images'] = False
    ...
    def process_images_and_project(self,
        ...
        images = images.to(self.img_mean.device)  # 移到和 img_mean 相同的设备上
