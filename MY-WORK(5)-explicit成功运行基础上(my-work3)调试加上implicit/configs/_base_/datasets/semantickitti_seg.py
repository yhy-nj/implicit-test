...
data_root = '/media/user/data/KITTI-Seg/'
class_names = [
...
backend_args = None
data_prefix = dict(
    pts='/media/user/data/KITTI-Seg/',
    img='/media/user/data/KITTI-Seg/',
    pts_semantic_mask='/media/user/data/KITTI-Seg/'
)
...
train_pipeline = [
    ...
    dict(
        ...
        dataset_type='semantickitti',
        backend_args=backend_args),
# In the dataset pipeline config, add:
    dict(type='LoadCalibration'),  # load lidar2img matrix
    dict(type='LoadImageFromFile'),  # or custom image loader
    ...
    dict(type='Pack3DDetInputs', keys=['points', 'pts_semantic_mask','imgs'],mata_keys = ['num_points','lidar2img','img_path'])
]
test_pipeline = [
    dict(
        type='LoadPointsFromFile',
...
# In the dataset pipeline config, add:
    ...
    dict(type='Pack3DDetInputs', keys=['points','imgs'],mata_keys = ['num_points','lidar2img','img_path'])
]
tta_pipeline = [
    dict(
        type='LoadPointsFromFile',
        ...
        transforms=[[
            ...
            dict(
                ...
                flip_ratio_bev_vertical=1.)
                    ...
                    [
                        # dict(type='Pack3DDetInputs',keys=['points'],meta_keys=['num_points'])
                        dict(type='Pack3DDetInputs',keys=['points','imgs'],meta_keys=['num_points','lidar2img','img_path'])
                    ]])
]
